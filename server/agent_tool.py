...
class AgentTool:
                    ...
                    for deserialized_data in unpacker:
                        ...
                        if self._is_file_transfer_message(message):
                            self._handle_file_transfer_message(
                                message, file_transfer_state, addr
                            )
                            continue
    ...
    def _handle_encrypted_message(
        self, data: bytes, rsa_chipher: PKCS1_OAEP.PKCS1OAEP_Cipher
    ) -> str:
    ...
    def _handle_structured_message(self, message_dict: dict, addr: tuple) -> None:
        ...
        if message_type == "screen_recording_start":
            ...
            logger.warning(
                f"Unknown structured message type: {message_type} from {addr}"
            )
    ...
    def _handle_screen_recording_start(self, message_dict: dict, addr: tuple) -> None:
        ...
        logger.info(
            f"Screen recording started from {addr}: {width}x{height} at {fps}fps for {duration}s"
        )
    ...
    def _handle_screen_frame(self, message_dict: dict, addr: tuple) -> None:
        ...
        if not hasattr(self, "screen_recordings") or addr not in self.screen_recordings:
            logger.warning(
                f"Received frame from {addr} but no active recording session"
            )
            return
        ...
        if frame_number % 30 == 0 or frame_number < 5:
            logger.info(
                f"Screen recording progress from {addr}: frame {frame_number}/{total_frames} (received {session['received_frames']} frames)"
            )
    ...
    def _handle_screen_recording_end(self, message_dict: dict, addr: tuple) -> None:
        ...
        session = self.screen_recordings[addr]
        logger.info(
            f"Screen recording ended from {addr}. Received {session['received_frames']} frames out of {session['total_frames']} expected"
        )

        if session["received_frames"] == 0:
            logger.error(
                f"No frames received for session {session['session_id']}. Cannot create video."
            )
        else:
    ...
    def _create_video_from_frames(self, session: dict, addr: tuple) -> None:
        ...
        logger.info(
            f"Creating video from {len(frames)} frames for session {session_id}"
        )
        ...
        try:
            ...
            logger.warning(
                f"Could not create recordings directory: {e}, using current directory"
            )
            recordings_dir = "."
        ...
        try:
            ...
            (
                ...
                .output(
                    output_path, vcodec="libx264", pix_fmt="yuv420p", loglevel="error"
                )
                .overwrite_output()
            ...
            if os.path.exists(output_path):
                ...
                logger.info(
                    f"Video created successfully: {output_path} ({file_size} bytes)"
                )
            else:
            ...
            logger.error(
                f"FFmpeg command failed. Check if ffmpeg is installed on the system."
            )
    ...
    def _handle_single_file_transfer(self, message_dict: dict, addr: tuple) -> None:
        ...
        try:
            ...
            logger.info(
                f"File received and saved: {output_filename} ({len(file_bytes)} bytes)"
            )
    ...
    def _handle_structured_file_chunk(self, message_dict: dict, addr: tuple) -> None:
        ...
        if chunk_type == "start":
            ...
            logger.info(
                f"File transfer started: {filename} ({total_size} bytes, {total_chunks} chunks)"
            )
            ...
            if hasattr(self, "file_transfers") and addr in self.file_transfers:
                ...
                if received_chunks % 10 == 0:
                    logger.info(
                        f"File transfer progress: {received_chunks}/{total_chunks} chunks"
                    )
    ...
    def _reconstruct_chunked_file(self, addr: tuple) -> None:
        ...
        logger.info(
            f"Chunked file reconstructed: {output_filename} ({len(full_data)} bytes)"
        )
    ...
    def _handle_file_transfer_message(
        self, message: str, file_transfer_state: dict, addr: tuple
    ) -> None:
        ...
        if file_transfer_state["active"]:
            ...
            if len(parts) == 3:
                ...
                if file_transfer_state["received_chunks"] % 50 == 0:
                    logger.info(
                        f"Received chunk {file_transfer_state['received_chunks']}/{file_transfer_state['total_chunks']}"
                    )
    ...
    def _cleanup_connection(
        self, agent: Agent, client_socket: socket, addr: tuple
    ) -> None:
